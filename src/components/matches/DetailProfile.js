...
export function DetailProfile({
  ...
  useEffect(() => {
    if (Array.isArray(image)) {
      setImageList(image); // ใช้ image ตรงๆ เพราะมันเป็น Array อยู่แล้ว
      setCurrentImageIndex(0);
    }
  }, [image]);
  ...
  return (
    <div className="h-full w-full">
      ...
      <div className="preview-card h-full w-full bg-white lg:w-[1140px] lg:rounded-[32px]">
        <form
          method="dialog"
            ...
            <div className="information flex flex-col gap-6 px-4 py-6 lg:w-full lg:gap-10 lg:pb-0 lg:pl-[60px] lg:pt-6">
              ...
              <div className="sexual-preference flex flex-col">
                <label className="sexual-identities flex flex-row gap-2 py-[6px]">
                  <span className="w-2/5 max-w-[10rem] text-base font-normal text-fourth-900">
                    Sexual identities
                  </span>
              ...
              <div className="about-me flex flex-col gap-3 lg:gap-4">
                ...
                <span className="overflow-hidden text-ellipsis text-base font-normal text-fourth-900">
                  {aboutMe}
                </span>
              </div>
              ...
              <div className="hobbies-interests flex flex-col gap-3 lg:gap-6">
                ...
                <div className="hobby-list flex flex-wrap items-center gap-3">
                  {hobbyList.map((option, index) => {
                    return (
                      <p
                        key={index}
                        className="whitespace-normal break-words rounded-lg border border-second-300 px-4 py-2 text-center text-second-500"
                      >
                        {option}
                      </p>
                    );
